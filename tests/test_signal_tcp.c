...
int main(void) {
    ...
    if (agorahex_signal_start(AGORAHEX_SIGNAL_CLIENT_MODE, g_server_ipv4_addr, port, client_cb, client_connect_status_cb) !=
        AGORAHEX_OK) {
    ...
    if (g_client_connected_count != 1 || g_client_connected_fd < 0 ||
        g_client_connect_status != AGORAHEX_SIGNAL_CLIENT_CONNECTED) {
        fprintf(stderr, "client: connect status callback mismatch count=%d fd=%d status=%d\n",
                g_client_connected_count, g_client_connected_fd, (int)g_client_connect_status);
        agorahex_signal_close();
        kill(pid, SIGKILL);
        waitpid(pid, NULL, 0);
        return 1;
