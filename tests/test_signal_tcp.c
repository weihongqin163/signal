...
static agorahex_signal_connect_status_t g_server_connect_status;
static int g_client_connected_count;
static int g_client_connected_fd = AGORAHEX_SIGNAL_BROADCAST_FD;
static int g_client_connected_fd_is_open;
static agorahex_signal_connect_status_t g_client_connect_status;
...
static void server_connect_status_cb(int fd, agorahex_signal_connect_status_t status) {
    g_server_disconnected++;
    g_server_disconnected_fd = fd;
    g_server_connect_status = status;
}

static void client_connect_status_cb(int fd, agorahex_signal_connect_status_t status) {
    g_client_connected_count++;
    g_client_connected_fd = fd;
    g_client_connected_fd_is_open = fcntl(fd, F_GETFD) >= 0;
    g_client_connect_status = status;
}

static int run_server(int port) {
    long long deadline = now_ms() + 5000ll;
    int rc =
        agorahex_signal_start(AGORAHEX_SIGNAL_SERVER_MODE, g_server_ipv4_addr, port, server_cb, server_connect_status_cb);
    if (rc != AGORAHEX_OK) {
    ...
    while (now_ms() < deadline) {
        ...
        if (g_server_reply_sent && g_server_disconnected == 1) {
            if (g_server_disconnected_fd != g_server_peer_fd ||
                g_server_connect_status != AGORAHEX_SIGNAL_DISCONNECT_PEER_CLOSED) {
                agorahex_signal_close();
                return 1;
            }
            agorahex_signal_close();
...
int main(void) {
    ...
    if (agorahex_signal_start(AGORAHEX_SIGNAL_CLIENT_MODE, NULL, port, client_cb, NULL) !=
        ...
        agorahex_signal_start(AGORAHEX_SIGNAL_CLIENT_MODE, malformed_ipv4_addr, port, client_cb, NULL) !=
            AGORAHEX_ERR_INVALID_ARG ||
        agorahex_signal_start(AGORAHEX_SIGNAL_SERVER_MODE, NULL, port, server_cb, server_connect_status_cb) !=
            AGORAHEX_ERR_INVALID_ARG ||
        agorahex_signal_start(AGORAHEX_SIGNAL_SERVER_MODE, malformed_ipv4_addr, port, server_cb,
                              server_connect_status_cb) != AGORAHEX_ERR_INVALID_ARG) {
    ...
    if (agorahex_signal_start(AGORAHEX_SIGNAL_CLIENT_MODE, g_server_ipv4_addr, port, client_cb,
                              client_connect_status_cb) != AGORAHEX_OK) {
    ...
    if (g_client_connected_count != 1 || g_client_connected_fd < 0 || !g_client_connected_fd_is_open ||
        g_client_connect_status != AGORAHEX_SIGNAL_CLIENT_CONNECTED) {
        fprintf(stderr, "client: connect status mismatch count=%d fd=%d open=%d status=%d\n",
                g_client_connected_count, g_client_connected_fd, g_client_connected_fd_is_open,
                (int)g_client_connect_status);
        agorahex_signal_close();
        kill(pid, SIGKILL);
        waitpid(pid, NULL, 0);
        return 1;
