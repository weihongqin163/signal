 ...
 * In server mode, it is called synchronously after an accepted client is closed and removed with one of the
 * AGORAHEX_SIGNAL_DISCONNECT_* statuses; fd is no longer valid and is only an application mapping key.
 * In client mode, it is called synchronously from agorahex_signal_poll() after a TCP connection completes with
 * AGORAHEX_SIGNAL_CLIENT_CONNECTED; fd is the active connected socket. The callback may be NULL.
 */
 ...
 *   status_cb        - optional connection lifecycle callback. In server mode it reports accepted-client
 *                      disconnections; in client mode it reports each completed TCP connection.
 * Returns:
